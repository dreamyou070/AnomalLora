...
def main(args):
    ...
    def get_img_masked_token(batch):
        object_position = batch['object_mask'].squeeze().flatten()
        object_position = torch.where(object_position > 0, 1, 0)
        object_position = object_position.unsqueeze(0).to(dtype=weight_dtype)
    ...
    for epoch in range(args.start_epoch, args.max_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.set_grad_enabled(True):
                ...
                image_atts = get_img_masked_token(batch)  # adding attention mask, [1, 64*64]
            ...
            for trg_layer in args.trg_layer_list:
                ...
                num_head = cls_score.shape[0]
                image_atts = image_atts.repeat(num_head, 1)

                normal_trigger_score = (trigger_score * image_atts).mean(dim=0)  # pix_num
                normal_trigger_scores = [score for i, score in enumerate(normal_trigger_score) if image_atts[i] > 0]
                normal_cls_score = (cls_score * image_atts).mean(dim=0)
                normal_cls_scores = [score for i, score in enumerate(normal_cls_score) if image_atts[i] > 0]
                total_score = torch.ones_like(normal_trigger_score)
            ...
            if args.do_anomal_hole:
                ...
                for trg_layer in args.trg_layer_list:
                    ...
                    total_score = torch.ones_like(cls_score)
                    anormal_cls_loss = generate_attn_loss(cls_score, anomal_position, total_score, do_lowering=False)
                    anormal_trigger_loss = generate_attn_loss(trigger_score, anomal_position, total_score,
                                                              do_lowering=True)
            ...
            if args.do_dist_loss:
                normal_dist_max, normal_dist_loss, mu, cov = gen_mahal_loss(args, anormal_feat_list, normal_feat_list,
                                                                            mu, cov)
                dist_loss += normal_dist_loss.to(weight_dtype).requires_grad_()
                if args.do_down_dim_mahal_loss:
                    down_dim_normal_dist_max, down_dim_normal_dist_loss, down_mu, down_cov = gen_mahal_loss(args,
                                                                                                            down_dim_anormal_feat_list,
                                                                                                            down_dim_normal_feat_list,
                                                                                                            None, None)
