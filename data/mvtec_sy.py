...
class MVTecDRAEMTrainDataset_Sub(Dataset):
    ...
    def __init__(self,
        ...
        for folder in folders:
            ...
            rgb_folder_dir = os.path.join(folder_dir, "rgb")
            imgs = os.listdir(rgb_folder_dir)
            for img in imgs:
                self.image_paths.append(os.path.join(rgb_folder_dir, img))
    ...
    def __getitem__(self, idx):
        ...
        parent, _ = os.path.split(parent)
        object_mask_dir = os.path.join(parent, f"object_mask/{name}")
        gt_mask_dir = os.path.join(parent, f"gt/{name}")
        ...
        # [3] gt mask
        gt_img = self.load_image(gt_mask_dir, self.latent_res, self.latent_res, type='L')
        ...
        sample = {'image': self.transform(img),
                  "object_mask": object_mask.unsqueeze(0),   # [1, 64, 64]
                  "anomaly_mask": gt_mask.unsqueeze(0),  # [1, 64, 64] ################################
                  'idx': idx,
                  'input_ids': input_ids.squeeze(0),
                  'caption': self.caption,}
